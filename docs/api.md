...
`check` checks to see if the file has been formatted with Prettier given those options and returns a `Boolean`. This is similar to the <!-- `--check` or --> `--list-different` parameter<!-- s --> in the CLI and is useful for running Prettier in CI scenarios.
